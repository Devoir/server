...
describe('Task Routes', function () {
	...
	describe('create', function() {
		...
		it ('should create a task', function(done) {
			...
			request(app)
				.post('/api/courses/' + newCourseId + '/tasks')
				.send(data)
				.expect(200)
		...
		it ('should create a third task', function(done) {
			...
			request(app)
				.post('/api/courses/' + newCourseId2 + '/tasks')
				.send(data)
				.expect(200)
				.end(function(err, res) {
					assert.isNull(err, err);
					assert(res.body.id, 'Should have gotten back an id');
					newTaskId3 = res.body.id;
					done();
				});
		});
	});
